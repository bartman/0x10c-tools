...
static char * x10c_append_arg(char *p, char *e, const char *prefix,
		x10c_word arg_desc, const x10c_word *word)
{
	x10c_word ofs;
	struct x10c_reg *reg;
		...
		char *buf, size_t buf_len, const x10c_op_t *op)
{
	...
	a_len = x10c_op_arg_len(op->b.a);

	p = x10c_append_arg(p, e, " ",  op->b.b, words + a_len);
